...
# Model parameters
group = parser.add_argument_group('Model parameters')#STN_net
group.add_argument('--STN_pkl', default='', type=str)
...
group.add_argument('--for-STN', action='store_true', default=True,
                    help='STN preprocessing')
...
group.add_argument('--no-ddp-bb', action='store_true', default=True,
...
def main():
    ...
    STN = Get_STN_model(pkl_file=args.STN_pkl,classes=args.num_classes)
    ...
    loader_train = create_loader(
        dataset_train,
        input_size=data_config['input_size'],#(3,384,384)
        batch_size=args.batch_size,
        is_training=True,
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            #eval_metrics = validate(STN,model, loader_eval, validate_loss_fn, args, amp_autocast=amp_autocast)
            if args.distributed and hasattr(loader_train.sampler, 'set_epoch'):
                loader_train.sampler.set_epoch(epoch)

            train_metrics = train_one_epoch(
                STN, epoch, model, loader_train, optimizer, train_loss_fn, args,
...
def Get_STN_model(pkl_file,classes=219,device="cuda"):
    #*******************************************************************************************
    model_name = "resnet50"
    ...
    STN_net.load_state_dict(torch.load(pkl_file))
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            rois_0,rois_1,rois_2,theta_x_y= STN(input)
            output_0 = model(rois_0)
            # output_1 = model(rois_1)
            output_2 = model(rois_2)
            
            
            loss = (loss_fn(output_0, target)+loss_fn(output_2, target))/2
...
def validate(STN,model, loader, loss_fn, args, amp_autocast=suppress, log_suffix=''):
    batch_time_m = AverageMeter()
    losses_m = AverageMeter()
    top1_m = AverageMeter()
    top5_m = AverageMeter()
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            loss = (loss_fn(output_0, target)+loss_fn(output_2, target))/2
            output = F.softmax(output_0, dim=1)+F.softmax(output_2, dim=1)
